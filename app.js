...
/* ==========================================================
   かんたん動画分割 - アプリ本体
   動画を約2分（最大1分59秒）ごとに、画質を落とさず自動分割する。
   処理はすべて端末内（ブラウザ内）で完結し、外部へ動画を送信しない。
   ========================================================== */

const SEGMENT_SECONDS = 119; // 2分ちょうどを超えないよう、1分59秒を上限にする
...
btnStart.addEventListener("click", async () => {
  ...
  if (currentFile.size > LARGE_FILE_WARN_BYTES) {
    showToast("大きな動画です。処理に時間がかかったり、端末のメモリが不足する場合があります。", "caution", 5000);
  }
  ...
  try {
    ...
    // 動画の長さ（今は結果には使わないが、極端に長い動画の目安表示に使える）
    await probeDuration(instance, inputName);
